...
def simulate_season(projetion_year, simulations, resume_season, verbose):
    # load dfs
    ...
    # move stats from current season to core_game_scoring_dict and core_team_scoring_dict
    if resume_season == True:
        core_game_scoring_dict = existing_skater_df.set_index('PlayerID')[['GP', 'Goals', 'Assists']].T.to_dict('list')
        core_team_scoring_dict = existing_team_df.set_index('Team')[['Wins', 'Losses', 'OTL', 'Goals For', 'Goals Against']].T.to_dict('list')
    else:
    ...
    # determine active rosters. we convert data from df to dict because the lookup times are faster (O(n) vs O(1))
    active_rosters = {}
    for team_abbreviation in monte_carlo_team_proj_df['Abbreviation']:
...
# Simulate season
simulate_season(projection_year, 1, False, True)
