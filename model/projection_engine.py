...
def player_monte_carlo_engine(skater_proj_df, core_player_scoring_dict, projection_year, simulations, download_files, verbose):
    ...
    monte_carlo_player_df = monte_carlo_player_df.merge(
        bootstrap_df[['PlayerID', 'gp_stdev', 'atoi_stdev', 'gper1kChunk_stdev', 'aper1kChunk_stdev']],
    ...
    for index, row in tqdm(monte_carlo_player_df.iterrows(), total=monte_carlo_player_df.shape[0], desc="Simulating Player Seasons"):
        ...
        for sim in range(simulations):
            sim_gp = min(max(np.random.normal(row['Games Played'], row['gp_stdev']), curr_gp), 82)
            if sim_gp <= 0:
                sim_ATOI = 0
                sim_Gper1kChunk = 0
                sim_Aper1kChunk = 0
            else:
                sim_ATOI = max(np.random.normal(row['ATOI'], row['atoi_stdev']), curr_toi/sim_gp)
                sim_Gper1kChunk = max(np.random.normal(row['Gper1kChunk'], row['gper1kChunk_stdev']), curr_g/(sim_ATOI*sim_gp/500))
                sim_Aper1kChunk = max(np.random.normal(row['Aper1kChunk'], row['aper1kChunk_stdev']), curr_a/(sim_ATOI*sim_gp/500))
